...
- 编辑器与 `publish --html-file/--content-file` 按当前账号 AppID 和文章文件绝对路径记录草稿关联。首次保存创建草稿，之后使用 `draft/update` 更新原稿；未修改时跳过上传与提交。
- 记录存放在 `%LOCALAPPDATA%\wechat-draft-publisher\drafts`，只保存草稿 ID、状态和内容指纹，不保存正文、AppSecret 或 access_token。文件路径变化或账号变化视为另一篇文章；已有草稿应先关联，不得按标题猜测匹配。
- 升级前已创建的草稿，可在编辑器的“已有草稿 ID”输入框填写，或使用 `publish --draft-id <media_id>` 首次关联。仅支持单篇草稿。
- 更新前读取远端草稿并比较上次同步指纹。后台修改、草稿被删除或读取失败时停止，不覆盖、不自动另建。不要通过删除关联记录绕过冲突。
- 提交使用跨进程文章锁；请求结果不确定时保留待确认状态并停止自动重试，避免重复创建。首次创建成功但响应丢失时，先到后台确认，再填写已有草稿 ID 关联。
- 若显示已保存但未完成校验，已知草稿 ID 会保留；后续停止自动更新，先核对后台并恢复关联记录。
- 页面按钮为“保存草稿”。关联已有草稿并保存意味着更新该草稿的内容；关联前确认 ID 属于当前文章。不能用此功能覆盖未确认的后台修改。
